...
export function ImageGallery({ images }: ImageGalleryProps) {
  ...
  return (
    <div className="flex items-center justify-center relative">
      <div className="relative h-[85vmin] w-[85vmin] max-h-[700px] max-w-[700px] overflow-hidden rounded-[30px]">
        {gsapReady &&
          images.map((image, i) => (
            <div
              key={image.url}
        ...
        <button
          className="absolute left-4 top-1/2 z-[101] flex h-12 w-12 sm:h-14 sm:w-14 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-2 border-black/20 bg-black/10 backdrop-blur-sm outline-none transition-all duration-300 ease-out hover:scale-110 hover:bg-black/20 hover:border-black/40 active:scale-95 focus-visible:ring-4 focus-visible:ring-black/30 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
          onClick={prev}
          disabled={disabled}
        ...
        <button
          className="absolute right-4 top-1/2 z-[101] flex h-12 w-12 sm:h-14 sm:w-14 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-2 border-black/20 bg-black/10 backdrop-blur-sm outline-none transition-all duration-300 ease-out hover:scale-110 hover:bg-black/20 hover:border-black/40 active:scale-95 focus-visible:ring-4 focus-visible:ring-black/30 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
          onClick={next}
          disabled={disabled}
    ...
    >
      {images.map((image, i) => (
        <g key={image.url} className="pointer-events-auto">
          ...
          <circle
            onClick={() => onSelect(i)}
            className="cursor-pointer fill-black/0 stroke-black/50 hover:stroke-black/100 transition-all"
            strokeWidth="2"
            cx={getPosX(i)}
            cy={getPosY()}
            r={circleRadius + 2}
